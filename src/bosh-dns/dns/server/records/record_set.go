...
func (r *RecordSet) Resolve(fqdn string) ([]string, error) {
	...
	finalRecords, err := r.Filter(aliasExpansions)
	if err != nil {
		return []string{}, err
	}
...
func (r *RecordSet) Filter(resolutions []string) ([]Record, error) {
	...
	for _, resolution := range resolutions {
		...
		// modifies health
		healthyRecords, unhealthyRecords := r.segregateIPs(hostRecords, resolution)
...
func (r *RecordSet) segregateIPs(records []Record, fqdn string) ([]Record, []Record) {
